...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        batch_xs, batch_ys, bacth_context = dataset.train.next_batch(batch_size)
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        # Fit training using batch data
        h_ae1_out = sess.run(ae.transform(),feed_dict={ae.x: batch_xs, ae.keep_prob: 0.9})
        temp = ae_2nd.partial_fit()
        cost, opt = sess.run(temp, feed_dict={ae_2nd.x: h_ae1_out, ae_2nd.keep_prob: 0.9})
...
for _ in range(1000):
    ...
    h_ae1_out = sess.run(ae.transform(), feed_dict={ae.x: batch_xs, ae.keep_prob: 0.8})
    h_ae2_out = sess.run(ae_2nd.transform(), feed_dict={ae_2nd.x: h_ae1_out, ae_2nd.keep_prob: 0.8})
...
for _ in range(1000):
    ...
    sess.run(train_step_ft, feed_dict={x_ft: batch_xs, y_: batch_ys,
                                      ae.keep_prob: 0.8, ae_2nd.keep_prob: 0.8})
